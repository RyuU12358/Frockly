...
import { parseRefToRange } from "../../formula/refUtils";
import {
  ensureFnRoot,
  ensureFnParamsCount,
  readFnParamsFromWorkspace,
} from "./blocklyOps";
...
export function BlocklyWorkspace({
  ...
  // Helpers extracted to blocklyOps.ts and refUtils.ts
  ...
  useEffect(() => {
    ...
    const loadXml = async (wsId: string) => {
      ...
      try {
        ...
        // ★ロード後に「無かったら入れる」を確実に
        ensureFnRoot(w, p, wsId);
        ensureFnParamsCount(w, p, wsId);
    ...
    (async () => {
      ...
      onWorkspaceApiRef.current?.({
        ...
        switchWorkspace: (wsId: string) => {
          ...
          if (wsRef.current) {
            ensureFnRoot(wsRef.current, projectRef.current, wsId);
            ensureFnParamsCount(wsRef.current, projectRef.current, wsId);
          }
        },
      ...
      const onBlocklyEvent = (e: Blockly.Events.Abstract) => {
        ...
        const params = readFnParamsFromWorkspace(ws, p, wsId);
